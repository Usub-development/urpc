...
namespace urpcv1
{
    ...
    inline uint8_t flags_set_transport(uint8_t f, uint8_t tp2bits) { return static_cast<uint8_t>((f & ~0b11u) | (tp2bits & 0b11u)); }
...
#pragma pack(pop)
    ...
    hdr_decode(std::span<const std::byte> buf)
    {
        ...
        h.len      = get_le32(ptr(i)); i += 4;
        h.ver      = static_cast<uint8_t>(*(ptr(i) + 0)); i += 1;
        h.type     = static_cast<uint8_t>(*(ptr(i) + 0)); i += 1;
        h.flags    = static_cast<uint8_t>(*(ptr(i) + 0)); i += 1;
        h.rsv      = static_cast<uint8_t>(*(ptr(i) + 0)); i += 1;
        h.stream   = get_le32(ptr(i)); i += 4;
        h.method   = get_le64(ptr(i)); i += 8;
        h.meta_len = get_le32(ptr(i)); i += 4;
        h.body_len = get_le32(ptr(i)); i += 4;
