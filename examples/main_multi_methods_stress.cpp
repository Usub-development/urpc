//
// Created by Kirill Zhukov on 30.11.2025.
//

#include <atomic>
#include <chrono>
#include <memory>
#include <span>
#include <string>
#include <string_view>
#include <vector>

#include "uvent/Uvent.h"
#include "uvent/system/SystemContext.h"
#include "ulog/ulog.h"

#include <urpc/client/RPCClient.h>
#include <urpc/utils/Hash.h>

using namespace usub;
using namespace usub::uvent;
using namespace std::chrono_literals;

static std::atomic<uint64_t> g_ok_calls{0};
static std::atomic<uint64_t> g_failed_calls{0};
static std::atomic<uint64_t> g_total_latency_ns{0};
static std::atomic<int> g_finished_clients{0};

constexpr int kClientsCount = 64;
constexpr int kRequestsPerClient = 16;

struct MethodCase
{
    const char* name;
    uint64_t id;
};

static constexpr MethodCase kMethods[] = {
    {"Example.Echo",    urpc::method_id("Example.Echo")},
    {"Example.Upper",   urpc::method_id("Example.Upper")},
    {"Example.Reverse", urpc::method_id("Example.Reverse")}
};
constexpr int kMethodsCount = 3;

static std::string to_upper(std::string_view s)
{
    std::string out{s};
    for (char& c : out)
        c = static_cast<char>(std::toupper(static_cast<unsigned char>(c)));
    return out;
}

static std::string reverse_str(std::string_view s)
{
    return std::string{s.rbegin(), s.rend()};
}

static uint32_t lcg(uint32_t& state)
{
    state = state * 1664525u + 1013904223u;
    return state;
}

usub::uvent::task::Awaitable<void> torture_client_multi(int client_id)
{
    ulog::info("CLIENT[{}]: torture_client_multi started", client_id);

    auto client = std::make_shared<urpc::RpcClient>("localhost", 45900);

    bool pong = co_await client->async_ping();
    ulog::info("CLIENT[{}]: ping={}", client_id, pong);

    uint32_t rnd_state = 0x12345678u ^ static_cast<uint32_t>(client_id * 7919);

    for (int i = 0; i < kRequestsPerClient; ++i)
    {
        uint32_t r = lcg(rnd_state);
        const MethodCase& m = kMethods[r % kMethodsCount];

        std::string payload_str =
            "client=" + std::to_string(client_id) +
            ";req=" + std::to_string(i) +
            ";method=" + std::string{m.name} +
            ";payload=abcdefghijklmnopqrstuvwxyz0123456789";

        std::span<const uint8_t> req{
            reinterpret_cast<const uint8_t*>(payload_str.data()),
            payload_str.size()
        };

        ulog::info("CLIENT[{}]: before call #{} method={}", client_id, i, m.name);

        auto started = std::chrono::steady_clock::now();
        auto resp = co_await client->async_call(m.id, req);
        auto ended = std::chrono::steady_clock::now();

        ulog::info("CLIENT[{}]: after call #{} method={}", client_id, i, m.name);

        auto ns = std::chrono::duration_cast<std::chrono::nanoseconds>(ended - started).count();
        g_total_latency_ns.fetch_add(static_cast<uint64_t>(ns), std::memory_order_relaxed);

        if (resp.empty())
        {
            g_failed_calls.fetch_add(1, std::memory_order_relaxed);
            ulog::error("CLIENT[{}]: empty response on req={} method={}", client_id, i, m.name);
            continue;
        }

        std::string resp_str(reinterpret_cast<const char*>(resp.data()), resp.size());
        std::string expected;

        if (std::string_view{m.name} == "Example.Echo")
        {
            expected = payload_str;
        }
        else if (std::string_view{m.name} == "Example.Upper")
        {
            expected = to_upper(payload_str);
        }
        else if (std::string_view{m.name} == "Example.Reverse")
        {
            expected = reverse_str(payload_str);
        }

        if (resp_str != expected)
        {
            g_failed_calls.fetch_add(1, std::memory_order_relaxed);
            ulog::error("CLIENT[{}]: mismatch on req={} method={} expected='{}' got='{}'",
                        client_id, i, m.name, expected, resp_str);
        }
        else
        {
            g_ok_calls.fetch_add(1, std::memory_order_relaxed);
        }
    }

    client->close();
    co_await system::this_coroutine::sleep_for(5ms);

    g_finished_clients.fetch_add(1, std::memory_order_relaxed);
    ulog::info("CLIENT[{}]: torture_client_multi finished", client_id);
    co_return;
}

int main()
{
    usub::ulog::ULogInit cfg{
        .trace_path = nullptr,
        .debug_path = nullptr,
        .info_path = nullptr,
        .warn_path = nullptr,
        .error_path = nullptr,
        .flush_interval_ns = 2'000'000ULL,
        .queue_capacity = 16384,
        .batch_size = 1024,
        .enable_color_stdout = true,
        .max_file_size_bytes = 10 * 1024 * 1024,
        .max_files = 3,
        .json_mode = false,
        .track_metrics = true
    };

    usub::ulog::init(cfg);
    ulog::info("STRESS MM: logger initialized");

    Uvent uvent(4);

    system::co_spawn([]() -> usub::uvent::task::Awaitable<void>
    {
        ulog::info("STRESS MM: pre-sleep 1s before spawning clients");
        co_await system::this_coroutine::sleep_for(1000ms);

        for (int i = 0; i < kClientsCount; ++i)
            system::co_spawn(torture_client_multi(i));

        while (true)
        {
            int finished = g_finished_clients.load(std::memory_order_relaxed);
            ulog::info("STRESS MM: finished_clients={}/{}", finished, kClientsCount);
            if (finished >= kClientsCount)
                break;
            co_await system::this_coroutine::sleep_for(100ms);
        }

        ulog::info("STRESS MM: all clients finished");
        co_return;
    }());

    ulog::info("STRESS MM: starting event loop");
    uvent.run();
    ulog::warn("STRESS MM: event loop finished");

    uint64_t ok = g_ok_calls.load(std::memory_order_relaxed);
    uint64_t failed = g_failed_calls.load(std::memory_order_relaxed);
    uint64_t total = ok + failed;

    if (total > 0)
    {
        uint64_t total_ns = g_total_latency_ns.load(std::memory_order_relaxed);
        double avg_us = static_cast<double>(total_ns) / static_cast<double>(total) / 1000.0;

        ulog::info("STRESS MM: total={} ok={} failed={} avg_latency_us={:.2f}",
                   total, ok, failed, avg_us);
    }
    else
    {
        ulog::info("STRESS MM: no calls were completed");
    }

    ulog::warn("STRESS MM: shutting down logger");
    usub::ulog::shutdown();
    return 0;
}